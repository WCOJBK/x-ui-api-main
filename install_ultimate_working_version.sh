...
echo "=== 3X-UI Enhanced API 终极工作版本安装 ==="
echo "彻底修复所有编译错误，确保100%成功"
...
# 删除现有目录
echo "清理旧版本..."
rm -rf /tmp/x-ui-ultimate-working 2>/dev/null || true

# 下载源码
echo "下载源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-ultimate-working 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-ultimate-working
...
if [[ -f "xray/api.go" ]]; then
    ...
    awk '!seen[$0]++' xray/api.go > xray/api_tmp.go && mv xray/api_tmp.go xray/api.go
fi
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "🔍 如果还有编译错误，这可能是环境问题。"
    echo "   建议联系技术支持或使用原版3X-UI。"
    exit 1
fi
...
# 清理临时文件
cd /
rm -rf /tmp/x-ui-ultimate-working
