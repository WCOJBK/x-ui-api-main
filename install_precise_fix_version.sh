...
echo "=== 3X-UI Enhanced API 精准修复版本 ==="
echo "精准修复main.go具体错误，确保全部API可用"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-precise-fix 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-precise-fix 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}
...
# 精准修复main.go中的具体错误
echo "精准修复main.go中的具体错误..."
if [[ -f "main.go" ]]; then
    echo "修复main.go第483行的removeSecret函数调用错误..."
    
    # 修复第483行的removeSecret调用错误 - 改为条件判断
