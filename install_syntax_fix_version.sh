...
echo "=== 3X-UI Enhanced API 语法修复版本 ==="
echo "修复main.go语法错误，确保编译成功"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-syntax-fix 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-syntax-fix 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}
...
# 修复main.go - 完全重写避免语法错误
echo "修复main.go文件（避免语法错误）..."
cat > main.go << 'EOF'
package main
...
func runWebServer() {
    // 设置配置文件路径
    configFile := flag.String("config", "/etc/x-ui/x-ui.conf", "config file path")
    flag.Parse()
    ...
    if config.GetCertFile() != "" && config.GetKeyFile() != "" {
        logger.Info("启用HTTPS，监听端口:", config.GetPort())
        server.TLSConfig = &tls.Config{
            MinVersion: tls.VersionTLS12,
        }
        
        go func() {
            if err := server.ListenAndServeTLS(config.GetCertFile(), config.GetKeyFile()); err != nil && err != http.ErrServerClosed {
                logger.Error("HTTPS服务器启动失败:", err)
            }
        }()
    } else {
        logger.Info("启用HTTP，监听端口:", config.GetPort())
        go func() {
...
func getCert() error {
    ...
    if err != nil {
        return fmt.Errorf("获取证书文件失败: %v", err)
    }

    keyFile, err := settingService.GetKeyFile()
    if err != nil {
        return fmt.Errorf("获取密钥文件失败: %v", err)
    }
