...
echo "=== 3X-UI Enhanced API 最终参数修复版本 ==="
echo "修复tgBot.Start方法参数不匹配问题，确保编译成功"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-final-parameter-fix 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-final-parameter-fix 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-final-parameter-fix
