...
echo "=== 3X-UI Enhanced API 超精准修复版本 ==="
echo "超精准修复removeSecret，区分函数声明和函数调用"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-ultra-precise 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-ultra-precise 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-ultra-precise
...
# 超精准修复main.go中的removeSecret错误
echo "超精准修复main.go中的removeSecret错误..."
if [[ -f "main.go" ]]; then
    echo "备份原始main.go..."
    cp main.go main.go.backup
    
    echo "分析removeSecret使用模式..."
    echo "搜索所有removeSecret相关行："
    grep -n "removeSecret" main.go | head -10
    
    echo "超精准修复策略："
    echo "1. 不修改函数声明（func removeSecret()）"
    echo "2. 不修改变量声明（removeSecret bool）"
    echo "3. 不修改参数引用（&removeSecret）"
    echo "4. 不修改条件判断（if removeSecret {）"
    echo "5. 只修复独立的函数调用（removeSecret()）"
    
    # 创建临时文件用于逐行处理
    temp_file="/tmp/main_go_fixed.tmp"
    > "$temp_file"
    ...
    while IFS= read -r line; do
        ...
        # 检查这一行是否包含removeSecret
        if echo "$line" | grep -q "removeSecret"; then
            # 检查是否是函数声明（包含func关键字）
            if echo "$line" | grep -q "^[[:space:]]*func.*removeSecret"; then
                echo "第${line_num}行: 保留函数声明"
                echo "$line" >> "$temp_file"
            # 检查是否是变量声明
            elif echo "$line" | grep -q "removeSecret[[:space:]]*bool"; then
                echo "第${line_num}行: 保留变量声明"
                echo "$line" >> "$temp_file"
            # 检查是否是参数引用
            elif echo "$line" | grep -q "&removeSecret"; then
                echo "第${line_num}行: 保留参数引用"
                echo "$line" >> "$temp_file"
            # 检查是否是条件判断
            elif echo "$line" | grep -q "if.*removeSecret[[:space:]]*{"; then
                echo "第${line_num}行: 保留条件判断"
                echo "$line" >> "$temp_file"
            # 检查是否是注释
            elif echo "$line" | grep -q "//.*removeSecret"; then
                echo "第${line_num}行: 保留注释"
                echo "$line" >> "$temp_file"
            # 检查是否是独立的函数调用
            elif echo "$line" | grep -q "removeSecret()"; then
                echo "第${line_num}行: 修复函数调用"
                # 替换独立的函数调用为注释
                fixed_line=$(echo "$line" | sed 's/removeSecret()/\/\/ removeSecret() function call removed/g')
                echo "$fixed_line" >> "$temp_file"
            else
                echo "第${line_num}行: 保持原样"
                echo "$line" >> "$temp_file"
            fi
        else
            # 不包含removeSecret的行，直接复制
            echo "$line" >> "$temp_file"
        fi
    done < main.go
    
    # 用修复后的内容替换原文件
    mv "$temp_file" main.go
    
    echo "移除未使用的导入包..."
    # 移除未使用的encoding/json导入
    sed -i '/^[[:space:]]*"encoding\/json"/d' main.go
    # 移除未使用的net/http导入  
    sed -i '/^[[:space:]]*"net\/http"/d' main.go
    
    echo "清理Telegram相关引用..."
    # 删除tgbot相关的导入和调用，不破坏整体结构
    sed -i '/import.*tgbot/d' main.go
    sed -i '/tgbot\./d' main.go
    sed -i '/".*tgbot"/d' main.go
    
    echo "验证修复结果..."
    echo "修复后的removeSecret相关行："
    grep -n "removeSecret" main.go | head -10
    
    echo "检查语法结构..."
    echo "检查func声明："
    grep -n "^[[:space:]]*func" main.go | head -5
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "🔍 检查main.go中的语法错误..."
    echo "1. 检查第350-360行："
    sed -n '350,360p' main.go
    echo ""
    echo "2. 检查第480-490行："
    sed -n '480,490p' main.go
    echo ""
    echo "3. 检查所有func声明："
    grep -n "^[[:space:]]*func" main.go | head -10
    echo ""
    echo "4. 搜索removeSecret相关："
    grep -n "removeSecret" main.go | head -10
