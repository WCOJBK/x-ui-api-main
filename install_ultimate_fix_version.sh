...
echo "=== 3X-UI Enhanced API 终极强力修复版本 ==="
echo "强力修复所有removeSecret错误，确保全部API可用"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-ultimate-fix 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-ultimate-fix 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}
...
# 强力修复main.go中的所有removeSecret错误
echo "强力修复main.go中的所有removeSecret错误..."
if [[ -f "main.go" ]]; then
    echo "搜索所有removeSecret函数调用错误..."
    
    # 使用强力方法修复所有removeSecret函数调用
    # 方法1：将所有removeSecret(...)调用改为条件判断
    sed -i 's/removeSecret([^)]*)/if removeSecret { \/\/ removeSecret enabled }/g' main.go
    
    # 方法2：修复可能的其他形式的removeSecret调用
    sed -i 's/removeSecret\s*(/if removeSecret { \/\/ } \/\//g' main.go
    
    # 方法3：针对具体行号的修复（多个可能的行号）
    sed -i '481s/removeSecret(.*)/if removeSecret { \/\/ removeSecret enabled }/' main.go
    sed -i '482s/removeSecret(.*)/if removeSecret { \/\/ removeSecret enabled }/' main.go
    sed -i '483s/removeSecret(.*)/if removeSecret { \/\/ removeSecret enabled }/' main.go
    sed -i '484s/removeSecret(.*)/if removeSecret { \/\/ removeSecret enabled }/' main.go
    sed -i '485s/removeSecret(.*)/if removeSecret { \/\/ removeSecret enabled }/' main.go
    ...
    # 额外的清理 - 移除任何包含removeSecret(的行并替换为注释
    sed -i 's/.*removeSecret(.*/\/\/ removeSecret function call removed/' main.go
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "🔍 检查main.go中的removeSecret错误..."
    echo "搜索包含removeSecret的行："
