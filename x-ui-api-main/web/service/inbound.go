...
// 高级客户端添加功能（支持流量限制、到期时间等）
func (s *InboundService) AddInboundClientAdvanced(inboundId int, client *model.Client) (bool, error) {
	...
	// 检查邮箱是否已存在
	existingClient, _ := s.GetClientTrafficByEmail(client.Email)
	if existingClient != nil {
		return false, common.NewError("Email already exists:", client.Email)
	}
	...
	case "shadowsocks":
		newClient["method"] = "aes-128-gcm" // 默认加密方法
		if client.Password != "" {
			newClient["password"] = client.Password
		}
	case "vless", "vmess":
		...
		if client.Security != "" {
			newClient["security"] = client.Security
		}
	}

	// 添加到设置中
	interfaceClients := settings["clients"].([]interface{})
	interfaceClients = append(interfaceClients, newClient)
	...
	if client.Enable {
		...
		if oldInbound.Protocol == "shadowsocks" {
			userConfig["cipher"] = settings["method"]
		}

		err1 := s.xrayApi.AddUser(string(oldInbound.Protocol), oldInbound.Tag, userConfig)
		if err1 != nil {
			logger.Debug("Error adding client via API:", err1)
			needRestart = true
		} else {
			logger.Debug("Client added via API:", client.Email)
		}
		s.xrayApi.Close()
	}
...
// 更新客户端高级设置
func (s *InboundService) UpdateClientAdvanced(email string, updates interface{}) error {
	db := database.GetDB()
	
	// 更新客户端流量统计
	err := db.Model(&xray.ClientTraffic{}).Where("email = ?", email).Updates(updates).Error
	if err != nil {
		return err
	}
