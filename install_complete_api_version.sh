...
echo "=== 3X-UI Enhanced API 完整API保障版本 ==="
echo "优先保障全部API可以使用，修复所有编译问题"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-complete-api 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-complete-api 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-complete-api
...
# 关键修复：不修改main.go，保持原始版本可以编译
echo "保持原始main.go以确保API完整可用..."
# 不重写main.go，使用原始版本

# 修复原始main.go中可能的Telegram引用
echo "清理main.go中的Telegram引用..."
if [[ -f "main.go" ]]; then
    # 只删除tgbot相关的导入和调用，不破坏整体结构
