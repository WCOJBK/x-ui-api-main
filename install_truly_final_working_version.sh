...
echo "=== 3X-UI Enhanced API 真正最终工作版本 ==="
echo "彻底修复类型转换问题，这是真正的最后一个版本"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-truly-final-working 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-truly-final-working 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-truly-final-working
