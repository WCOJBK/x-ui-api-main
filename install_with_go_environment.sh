...
echo "=== 3X-UI Enhanced API 完整环境安装 ==="
echo "自动安装Go环境 + 编译增强版本"
...
if command -v go >/dev/null 2>&1; then
    ...
    if [[ "$CURRENT_GO_VERSION" == "go1.21."* ]] || [[ "$CURRENT_GO_VERSION" == "go1.22."* ]]; then
        echo "✅ Go已安装: $CURRENT_GO_VERSION"
        GO_INSTALLED=true
    else
        echo "⚠️  Go版本过旧: $CURRENT_GO_VERSION, 需要升级"
    fi
fi
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-with-go-env 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-with-go-env 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}
...
// Telegram Bot存根服务 - 移除所有Telegram依赖
type Tgbot struct {
}
