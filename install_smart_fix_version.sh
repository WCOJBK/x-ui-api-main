...
echo "=== 3X-UI Enhanced API 智能精确修复版本 ==="
echo "智能修复removeSecret函数调用，保留变量和函数声明"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-smart-fix 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-smart-fix 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}
...
# 智能精确修复main.go中的removeSecret错误
echo "智能精确修复main.go中的removeSecret错误..."
if [[ -f "main.go" ]]; then
    echo "备份原始main.go..."
    cp main.go main.go.backup
    
    echo "搜索并分析removeSecret相关代码..."
    echo "当前removeSecret相关行："
    grep -n "removeSecret" main.go | head -10
    
    echo "智能修复removeSecret函数调用..."
    
    # 方法1：只修复形如 removeSecret() 的函数调用，保留其他用法
    # 使用更精确的正则表达式，只匹配函数调用形式
    sed -i 's/\bremoveSecret()/\/\/ removeSecret() function call removed/g' main.go
    
    # 方法2：修复可能的嵌套条件判断错误
    # 查找形如 "if removeSecret { // removeSecret enabled }" 的错误语法并修复
    sed -i 's/if removeSecret { \/\/ removeSecret enabled }/\/\/ removeSecret condition removed/g' main.go
    
    # 方法3：修复可能被破坏的函数声明
    # 查找形如 "func if removeSecret {" 的错误并修复为注释
    sed -i 's/func if removeSecret { \/\/ removeSecret enabled }/\/\/ removeSecret function declaration removed/g' main.go
    ...
    echo "检查是否还有语法错误的模式..."
    if grep -q "func if" main.go; then
        echo "⚠️  仍然存在 'func if' 模式，进行额外修复..."
        sed -i 's/func if.*/\/\/ function declaration syntax error fixed/g' main.go
    fi
    
    if grep -q "if.*if.*{.*}.*{" main.go; then
        echo "⚠️  仍然存在嵌套if语法错误，进行修复..."
        sed -i 's/if.*if.*{.*}.*{/\/\/ nested if syntax error fixed {/g' main.go
    fi
    
    echo "✅ main.go智能精确修复完成"
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "搜索可能的语法错误模式："
    echo "1. 检查func关键字："
    grep -n "func.*if" main.go | head -5
    echo "2. 检查嵌套if语句："
    grep -n "if.*if.*{" main.go | head -5
    echo "3. 检查removeSecret相关："
    grep -n "removeSecret" main.go | head -10
    echo "4. 检查第350-360行："
