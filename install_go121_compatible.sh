...
echo "=== 3X-UI Enhanced API Go1.21兼容版本安装 ==="
echo "自动安装Go环境 + 使用兼容依赖版本"
...
# 删除旧版本
echo "清理旧版本..."
rm -rf /tmp/x-ui-go121-compatible 2>/dev/null || true

# 下载源码
echo "下载3X-UI Enhanced API源码..."
cd /tmp
git clone https://github.com/WCOJBK/x-ui-api-main.git x-ui-go121-compatible 2>/dev/null || {
    echo "❌ Git clone 失败"
    exit 1
}

cd x-ui-go121-compatible
